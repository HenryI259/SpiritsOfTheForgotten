    ...
    public class Helper
    {
        ...
        public static class NPCs
        {
            ...
            public static int FindNearestNPC(Vector2 position, float maxDistance = 999999f)
            {
                ...
                float distanceFromTarget = maxDistance;
                ...
                for (int i = 0; i < Main.maxNPCs; i++)
                {
                    ...
                    {
                        float between = Vector2.Distance(npc.Center, position);
                        bool closest = Vector2.Distance(position, targetCenter) > between;
            ...
            /// <summary>
            public static NPC FindNearestNPCDirect(Vector2 position, float maxDistance)
            {
                Vector2 targetCenter = position;
                float distanceFromTarget = maxDistance;
                bool foundTarget = false;
        ...
        public static class Colors
        {
            ...
            public static Color LoreItem(float speed = 1f)
            {
                return Colors.lerp3(Color.Teal, Color.SteelBlue, Color.LightBlue, Numbers.ZerotoOne(1, 2));
            }
            ...
            public static Color lerpPlus(Color[] colors, float amount)
            {
                ...
                {
                    if (amount > (i * amount / colors.Length) && amount < ((i + 1) * amount / colors.Length))
                    {
                        if (i != colors.Length)
