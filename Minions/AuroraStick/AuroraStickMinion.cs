...
using SpiritsOfTheForgotten.Minions.AuroraStick;
using System;
...
namespace SpiritsOfTheForgotten.Minions.AuroraStick
{
	...
	public class AuroraStickMinion : ModProjectile
	{
		...
		public override void AI()
		{
			...
			{
				player.ClearBuff(ModContent.BuffType<AuroraStickBuff>());
			}
			if (player.HasBuff(ModContent.BuffType<AuroraStickBuff>()))
			{
			...
			if (Main.myPlayer == player.whoAmI && distanceToIdlePosition > 2000f)
			{
				...
				for (int i = 0; i < 20; i++)
				{
					int dusttype = Main.rand.Next(new int[] { 61, 62 });
			...
			if (foundTarget)
			{
				...
				stuck = projectile.Colliding(projectile.Hitbox, target.Hitbox);
				if (stuck && Main.player[projectile.owner].Distance(target.Center) < 2500f)
				{
