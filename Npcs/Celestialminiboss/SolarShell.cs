...
{
	public class SolarShell : ModNPC
	{
        ...
        public override void AI()
        {
			...
			npc.Center = playerTarget.Center - Vector2.UnitY.RotatedBy(MathHelper.ToRadians((spinTimer * 0.5f))) * 400;
			npc.rotation = (playerTarget.Center - npc.Center).ToRotation() + MathHelper.ToRadians(225);
			if (!Main.player[npc.target].HeldItem.ranged && !Main.player[npc.target].HeldItem.magic && !Main.player[npc.target].HeldItem.summon)
			{
				if (spinTimer % 180 == 0)
				{
					Vector2 speed = playerTarget.Center - npc.Center;
					AdjustMagnitude(ref speed, 5);
					if (Main.netMode != NetmodeID.MultiplayerClient)
					{
						owner = Main.myPlayer;
					}
					var a = Projectile.NewProjectileDirect(npc.Center, speed, ModContent.ProjectileType<SolarOrb>(), 15, 2, owner);
					a.friendly = false;
					a.hostile = true;
				}
			}
		...
		{
			Item.NewItem(npc.getRect(), ItemID.FragmentSolar, Main.rand.Next(4, 6));
			Item.NewItem(npc.getRect(), ItemID.FragmentSolar, Main.rand.Next(4, 6));
			Item.NewItem(npc.getRect(), ItemID.FragmentSolar, Main.rand.Next(4, 6));
		}
