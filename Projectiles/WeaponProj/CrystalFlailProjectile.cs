...
{
	public class CrystalFlailProjectile : ModProjectile
	{
		// The folder path to the flail chain sprite
		private const string ChainTexturePath = "SpiritsOfTheForgotten/Projectiles/WeaponProj/CrystalFlailProjectileChain";

		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Crystal Flail Ball"); // Set the projectile name to Example Flail Ball
		}
		...
		public override void AI()
		{
			...
			var dust = Dust.NewDustDirect(projectile.position, projectile.width, projectile.height, 172, projectile.velocity.X * 0.4f, projectile.velocity.Y * 0.4f, 61, default, 1.5f);
			dust.noGravity = true;
			dust.velocity /= 2f;
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
			if (crit)
			{
				for (int i = 0; i < 3; i++)
				{
					...
					int a = Projectile.NewProjectile(target.Center.X, target.Center.Y, (float)Math.Cos(angle * Math.PI / 180) * 2, (float)Math.Sin(angle * Math.PI / 180) * 2, ProjectileID.CrystalStorm, damage, 0, projectile.owner);
